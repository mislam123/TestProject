...
public class ReportFactory{
	...
	private synchronized static ExtentReports getExtentReport() {
		if (reporter == null) {
			reporter = new ExtentReports("TestResult/"+"AutomationReport.html", true, DisplayOrder.NEWEST_FIRST);
			reporter.config().documentTitle("Company Name"); 
			reporter.config().reportName("QA - ");
			reporter.config().reportHeadline("Automation Report");
		}
		return reporter;
	}
